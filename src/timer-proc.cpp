...
namespace KbAsMouse {
	VOID CALLBACK TimerProc(HWND hWnd, UINT uMsg, UINT_PTR idEvent, DWORD time) {
		...
		// Static is important since we access the memory outside of this function
		// from SendInput.
		...
		if (timeOfPrevFrame ==
			0) {	// beginning timer; we need to replace elapse time
		...
		double accelerationModifier =
			state.isVKKeyDown[state.slowKey] ? state.slowRatio : 1;
		...
		// velocity
		std::pair<double, double> mouseAcceleration =
			std::make_pair(-state.mouseVelocity.first *
					std::min(1 / timeDeltaS, state.mouseResistance),
				-state.mouseVelocity.second *
					std::min(1 / timeDeltaS, state.mouseResistance));
		...
		if (state.isVKKeyDown[state.upKey])
			mouseAcceleration.second -=
				state.mouseAcceleration * accelerationModifier;
		...
		if (static_cast<int>(state.mousePositionRemainder.first) != 0 ||
			...
			state.mousePositionRemainder.first -=
				static_cast<int>(state.mousePositionRemainder.first);
			state.mousePositionRemainder.second -=
				static_cast<int>(state.mousePositionRemainder.second);
		}
		...
		double scrollAcceleration =
			-state.scrollVelocity * std::min(1 / timeDeltaS, state.scrollResistance);
		...
		if (static_cast<int>(state.scrollPositionRemainder) != 0) {
			...
			state.scrollPositionRemainder -=
				static_cast<int>(state.scrollPositionRemainder);
		}

		// send input if we have something to send (either mousemove or
		// mousewheel)
		if (inputEv.mi.dwFlags != NULL) SendInput(1, &inputEv, sizeof(inputEv));

		// if velocity is not below threshold or if one of the wheel keys is down,
		// then continue the timer and try to maintain the framesPerSecond; else,
		// kill timer
		...
		if (abs(state.scrollVelocity) < VELOCITY_ZERO_EPS) state.scrollVelocity = 0;
		if (state.mouseVelocity.first == 0 && state.mouseVelocity.second == 0 &&
